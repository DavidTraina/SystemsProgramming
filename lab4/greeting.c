...
int main(int argc, char **argv) {
    ...
    // Your code goes here
    greeting[19] = '\0';
    strncpy(greeting, argv[1], 19); // strncpy meerly copies and does not add \0
    size_t space_left = 19 - strlen(greeting); // greeting[19] reserved for \0
    if (space_left >= 1) {
        greeting[strlen(greeting)] = ' '; // Notice single quotes
        space_left--;
    }
    strncat(greeting, name, space_left); // concatonates at most space_left visible characters, then adds the \0. This is why we saved room when calculating space_left
